...
int main(){
    ...
    long double u[Nx][Ny]; 
    long double v[Nx][Ny];
    fftwl_complex v_hat[Nx][Ny/2+1];
    fftwl_complex u_hat[Nx][Ny/2+1];

    long double wx[Nx][Ny];
    long double wy[Nx][Ny];
    long double gx[Nx][Ny];
    long double gy[Nx][Ny];
    fftwl_complex wx_hat[Nx][Ny/2+1];
    fftwl_complex wy_hat[Nx][Ny/2+1];
    fftwl_complex gx_hat[Nx][Ny/2+1];
    fftwl_complex gy_hat[Nx][Ny/2+1];
    ...
    for(int iteration_time=0;iteration_time<n_timesteps;iteration_time++){
        ...
        for(int i=0;i<Nx;i++){
            for(int j=0;i<Ny/2+1;j++){
                // Real and complex parts stored in the [0] and [1] parts
                u_hat[i][j][1] = (kx[i]*g_hat[i][j][0]+ky[j]*w_hat[i][j][0])/k2[i][j];
