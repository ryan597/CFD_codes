...
[xx,yy,C,C_true,res_it]=test_poisson_jacobi(Sample_Size,omega,aspect_ratio);
...
Xval = [Xvec,Yvec];
% [beta,R,J,CovB,MSE,ErrorModelInfo] = nlinfit(Xval,Ydata,@(omega, X) nls_poisson_jacobi(omega, X),beta0,options);
[beta, MSE, R] = gauss_newton(beta0, Ydata, Sample_Size, @fun);
...
function Cvec = fun(t)
    Sample_Size  = 50; 
    aspect_ratio = 2;
    [xx,yy,C] = test_poisson_jacobi(Sample_Size, t, aspect_ratio);
    Cvec = reshape(C(1:4:end, 1:4:end), [], 1);
end
