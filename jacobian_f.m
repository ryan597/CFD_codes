% ---------------------- Jacobian
function J = jacobian_f(beta,model,X,yfit)

fdiffstep = 6.0555e-06;
    function yplus = call_model_nested(betaNew)
        yplus = model(betaNew, X);
    end
J = statjacobianF(@call_model_nested, beta, fdiffstep, yfit);
end % function getjacobian

function J = statjacobianF(func, theta, DerivStep, y0, rowIdx)
